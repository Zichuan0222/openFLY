...
int exporttotxt(std::vector<double> exportvector1, std::vector<double> exportvector2, std::vector<double> exportvector3){

  std::ostringstream filenameVH;
  filenameVH << "/home/zichuan/openFLY/data/V1H1_lifetime1.txt";
  std::string filename = filenameVH.str();
...
double complex_lifetime(double const& temp){
  ...
  private:
    bool reset = false;
    double lim = 0.1;           // tolerance value
    double E_trap = -1731.56;  // v2 = -1726.05;  // v3 = -1720.68;  // V1 = -1731.56;      // V2V3 might be different
  ...
  for (int i = 0; dtrap < 6; i++) {
    timeit("TOTAL", [&] {
      ...
      auto const& [m, atom, dt, basin, changed] 
        = timeit("Super::choice", [&] { return sb.kmc_choice(rng); });
...
int main(){
  ...
  int No_iteration = 100;                                    // Setting no. of iterations for each temperature
  ...
  std::vector<double> meanVHlifetime;
  std::vector<double> stdevVH; 

  // iterate over a range of temperatures
  for (int k=0; k<temperature.size(); ++k){

    std::vector<double> VHlifetime;          
    for (int i=0; i<No_iteration; ++i){                                 
      VHlifetime.push_back(complex_lifetime(temperature[k]));           // Call the above function to calculate lifetime
      std::cout << "------------------------- END of iteration no." << i+1 << " at T = " << temperature[k] <<" K ---------------------------\n\n"; 
    }

    std::sort(VHlifetime.begin(), VHlifetime.end());                // Sort and print liftimes in ascending order
    std::cout << "\n\nliftimes: ";
    for (int j=0; j<VHlifetime.size(); ++j){
      std::cout << VHlifetime[j] << ", ";
    }     
  
    // Calculating the mean, std dev of VH at this temp
    double sum = std::accumulate(VHlifetime.begin(), VHlifetime.end(), 0.0);
    double meanlifetime = sum / VHlifetime.size();
    double stdev = (VHlifetime[int(No_iteration * 3/4)] - VHlifetime[int(No_iteration/4)]) / 1.35;    // Estimate stdev from interquartile range 
    

    // std::vector<double> squareterms(VHlifetime.size());
    // for (int i=0; i<VHlifetime.size(); ++i){
    //   squareterms[i] = (VHlifetime[i] - meanlifetime)*(VHlifetime[i] - meanlifetime);}
    // double sq_sum = std::accumulate(squareterms.begin(), squareterms.end(), 0.0);
    // double stdev = std::sqrt(sq_sum / VHlifetime.size());

    fmt::print("\nMean lifetime: {} s\n", meanlifetime);
    fmt::print("Std dev of lifetimes: {} s \n", stdev);

    meanVHlifetime.push_back(meanlifetime);
    stdevVH.push_back(stdev);

  }

  exporttotxt(temperature, meanVHlifetime, stdevVH);


  return 0;
};

//    T range from 200K to about 1000K
//    25 iterations for each T
//    Std dev code
//    V2 manual check disscociations 

////// Notes for the code:
//  --- shall I store the bin file in case of corruption?
//  --- then we could reuse previously stored bin files when error come up?
